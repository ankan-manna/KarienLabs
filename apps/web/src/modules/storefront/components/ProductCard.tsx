    ...
    <Card className="group flex flex-col overflow-hidden transition-shadow hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        ...
        <Link
          ...
          className="line-clamp-2 text-sm font-medium text-charcoal-teal hover:text-healthcare-teal dark:text-gray-100"
        >
          {product.name}
        </Link>
        {!!product.ratingCount && (
        ...
        <div className="mt-1 flex items-baseline gap-2">
          <span className="text-base font-semibold text-charcoal-teal dark:text-gray-100">
            {formatCurrency(product.basePrice)}
          </span>
          {discountPct > 0 && (
            <span className="text-xs text-slate-teal line-through">{formatCurrency(product.mrp)}</span>
          )}
        </div>
