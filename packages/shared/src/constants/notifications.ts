export const NOTIFICATION_CHANNELS = {
  ...
  SMS: 'sms',
  PUSH: 'push',
  // Added for  10's OTP delivery-channel requirement — otherwise a
...
export const NOTIFICATION_STATUS = {
  ...
  SENT: 'sent',
  FAILED: 'failed',
  // a notification that was NEVER enqueued to the dispatch
...
export const CONFIGURATION_NAMESPACES = {
  ...
  GST: 'gst',
  CMS: 'cms',
  // AWS S3 / MinIO credentials + retention-days knobs (Part 28).
  // Pre-existing gap found and fixed in  27: `s3.client.ts` has always
  ...
  S3: 's3',
  // OTP + admin Google-login knobs, admin-editable via the
  ...
  // prescription/medical-compliance feature toggles (Part 38).
  // See modules/customers/prescription-config.service.ts.
  PRESCRIPTION: 'prescription',
  // coupon/promotion management feature toggles (Part 8/52).
  // See modules/coupons/coupon-config.service.ts.
  COUPON: 'coupon',
  // notification channel/category enable-disable toggles
  // (Part 14/15/47). See modules/notifications/notification-config.service.ts.
  NOTIFICATION: 'notification',
  // per-domain admin analytics/BI enable-disable toggles
  // (Part 2/3/38/46). See modules/admin/analytics-config.service.ts.
  ANALYTICS: 'analytics',
  // Search/SEO/AEO/GEO feature toggles (Part 16/17/49).
  // See modules/search/seo-config.service.ts.
  SEO: 'seo',
  // automated post-payment fulfillment (6-hour sweep) toggles
  // and cadence (Part 30). See modules/orders/fulfillment-config.service.ts.
  FULFILLMENT: 'fulfillment',
  // customer address pincode-validation / mobile-OTP /
